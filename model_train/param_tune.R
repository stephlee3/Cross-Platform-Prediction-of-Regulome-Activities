...
exon.data=as.matrix(read.table("exon_match.txt",header=T))
#micro.data=as.matrix(read.table("micro_match.txt",header=T))
id.test=c(8,9,24,43,45,47,52)
id.train=as.matrix(c(1:57))[-id.test]


load("DNase_data.rda")  #input DNase-seq data, first three columns contain the genomic locus information 
DNase_data=DNase_data[,-(id.test+3)]
...
group_idx <- sample(cut(seq(1,length(id.train)),breaks=5,label=FALSE))  #randomly partition the data into 5 groups

Num_cluster <- c(100,200,500,1000,1500,2000) #set range of K
Num_predictor <- c(1:8)  #set range of N

for(m in 1:8){
  sel.gene=which(cor.tmp>p[m])
  Exon_data=exon.data[sel.gene,]
  ...
  filename1=paste0("cross_validation_result_final_",m,".txt")
  filename2=paste0("best_parameter_final_",m,".txt")
  
  write.table(evaluate_result,file=filename1,sep="\t") #record the prediction performance for all K,N pairs
  write.table(c(N_max,K_max),file=filename2) #record the optimal K,N
